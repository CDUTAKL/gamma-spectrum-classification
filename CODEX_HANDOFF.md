...
项目最早先把 820 通道伽马能谱三分类的完整训练链路跑通，先建立一个可复现、可对照的 flat baseline。这个阶段重点是 TriBranch CNN、GradientBoosting、XGBoost、OOF cache、Phase 2 stacking 和 artifacts 导出，把“能稳定训练、能稳定评估”做实。此时项目的目标还很单纯，核心是先得到一个可靠的总分平台，而不是讨论复杂分层策略。

- 关键 commit：`f807ae7`
- 含义：恢复当时最佳 round2 baseline，最接近历史上 `Acc≈0.7762` 的 flat 代码线
- 阶段定位：项目基线，用作后续所有方案的对照锚点

#### 5.5.2 瓶颈识别期
在 flat baseline 稳定之后，项目逐渐进入高平台期：总 `Acc` 继续提升变难，但粉土这一 hardest class 一直拖累 `Macro-F1`。这时研究重点开始从“继续堆总分”转向“针对粉土边界问题做结构性改进”。这个阶段的关键变化不是代码大重构，而是对问题本质的重新判断。也正是在这里，项目开始明确：真正卡住的不是训练流程，而是类别边界，尤其是粉土边界。

- 这一阶段没有单一 commit 能完整代表，但它是后续 hierarchical 思路的直接前提
- 核心认知：真正的瓶颈不是训练不通，而是粉土边界样本难分

#### 5.5.3 方案 A 引入期：hierarchical stacking
项目首次把 hierarchical 思想正式引入最终判决层，核心是把三分类拆成两层：先判“粉土 vs 非粉土”，再在非粉土内部判“粘土 vs 砂土”。这一阶段的重点是验证分层判决本身有没有价值，而不是一上来就追求极限指标。它标志着项目从“纯 flat 三分类”转向“承认 hardest class 需要单独处理”的路线。方案 A 的雏形就在这里出现。
...
后续项目意识到，hierarchical 是否有效，关键不只是“有没有分层”，而是 Stage 1 到底放多少样本进粉土。因此引入阈值控制，开始系统 sweep `silt_threshold`。这一变化让方案 A 从一个结构想法，变成了一个可精修的决策系统。真正决定表现的，不再只是“分不分层”，而是“粉土入口开多大”。
...
#### 5.5.5 方案 A 定稿期
经过多轮 `Phase2-only + OOF cache` sweep，以及完整训练复验，方案 A 的角色被明确：它不是为了极限 `Acc`，而是更适合粉土 `F1 / Macro-F1` 的主线。当前最稳定的配置是 `hierarchical + threshold + 0.43`。这时项目已经形成非常清晰的结论分工：flat baseline 负责总分对照，hierarchical 负责边界类改善。换句话说，方案 A 已经不是实验性想法，而是成熟可复用方案。
...
#### 5.5.6 B1 初版：ML 分支层级训练
在方案 A 基本吃透后，项目尝试把 hierarchical 思想前推到 ML 分支训练，形成 B1。目标是让 GB / XGB 也按层级任务学表示，而不仅仅在最终判决层做分层。这一步的动机很自然：如果分层判决有效，那是否应该让前面的基模型也按同样逻辑训练？B1 初版就是围绕这个问题展开的最小原型。

- 关键 commit：`7495580`
- 含义：加入 B1 hierarchical ML branch prototype
- 阶段定位：B1 原型期，验证“把层级思想前推到 ML 分支”是否值得

#### 5.5.7 B1 增强期
B1 初版之后，项目没有马上放弃，而是按文献和结构判断进一步增强了它。增强内容主要包括三块：Stage1 / Stage2 独立重采样、staged uncertainty / staged meta features、class-balanced weighting。这个阶段的目标很明确，就是给 B1 一个公平、完整的机会，而不是只试一个很薄的原型版本。B1 在这一阶段已经不只是“改个训练入口”，而是变成了一个认真做过分层不平衡处理的实验线。
...
#### 5.5.8 B1 失败收口期
B1 初版和增强版都没有稳定超过方案 A，说明只改 ML 分支还不够，hardest class 的核心瓶颈可能更偏向 CNN 表征层，而不是 GB/XGB 后端。这个结论很重要，因为它帮项目排除了一条“看起来合理，但收益不够”的路径。B1 不是没价值，它的价值在于把“只改后端不够”这件事验证清楚了。到这一步，继续细抠 B1 已经不划算，项目自然应该收口。
...
#### 5.5.9 工程化增强期
为了让研究迭代更稳，项目同步补强了工程链路，包括配置拆分、结果摘要、样本降权跨平台修复、artifact 输出可读性和训练性能优化。这些改动不直接改变研究路线，但显著改善了实验可复现性和跨机器一致性。尤其是本机和 5090 之间的路径、权重、日志与结果输出问题，被逐步规整掉。这样后续无论是回溯旧版本，还是切换新实验线，都更可靠。
...
#### 5.5.10 当前阶段结论
当前项目已经形成很清楚的分层结论：flat baseline 是 Acc 对照锚点，方案 A 是粉土 F1 / Macro-F1 的成熟主线，B1 则完成了验证并收口。再往前走，最可能的下一步不是继续修 B1，而是考虑 B2，把层级思想真正前推到 CNN 表征层。这个判断不是说 B2 一定成功，而是说它比继续微调 B1 更值得尝试。就当前状态看，项目已经从“找一个能跑的模型”演化为“围绕 hardest class 做结构性优化”的稳定研究线。
